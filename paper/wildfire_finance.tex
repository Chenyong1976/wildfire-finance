...
This paper makes three contributions. First, it provides the first causal estimates
of wildfire effects on county government fiscal outcomes outside California,
extending \citet{liao2022} from California municipalities to eight western states
with varying property tax institutions, evaluated at both the extensive (incidence)
and intensive (severity) margins. Second, it demonstrates that defining treatment
by fire severity rather than fire incidence improves the identification environment:
the expanded low-to-moderate-severity control pool reduces observable selection and
yields cross-estimator sign agreement. Third, the null result is itself
informative: point estimates are uniformly negative, but the quinquennial
observation window lacks power to resolve effects of the order suggested by
the point estimates; annual CoG data would be the natural next step.
...
The null test results should be read as consistent with, not confirmation of,
...
intergovernmental transfers: ATT = \$\!-841 (SE = 661, $p = 0.203$), suggesting
that federal and state aid does not offset own-source revenue losses in the
medium run. Property tax revenue is near zero (\$+46, $p = 0.824$), consistent
...
$p = 0.139$). For g\,=\,2022 counties, fiscal balance is \$\!-211 (SE = 187,
...
The g\,=\,2022 cohort (first observed post-treatment in 2022, a single census year)
yields very imprecise estimates: fiscal balance \$\!-865 (SE = 1,420) from a
single post-treatment observation. Across both designs, the capital outlay
...
the never-treated baseline. A 10-fold increase in max fire size (e.g., 10,000 to
...
Table~\ref{tab:robustness} presents five robustness specifications for fiscal balance,
...
All five specifications are directionally negative and statistically insignificant.
...
This paper provides the first causal estimates of wildfire fiscal effects on county
governments outside California, evaluated along both the extensive margin (fire
incidence) and the intensive margin (fire severity). Using Census of Governments
quinquennial data from 2002 to 2022 and WFP-matched staggered DiD designs, I find
uniformly negative but statistically insignificant effects on fiscal balance across
both margins. Capital expenditures provide the most consistent signal (\$\!-226 per
...
major hurricane estimates. Annual CoG survey data---with more post-treatment
observation windows---would substantially improve precision. The negative
...
year. Dashes indicate outcomes not estimated in the cohort decomposition run.
Sev.\ = high-severity.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
fires. None of the specifications yield statistical significance at conventional
levels; all five are directionally negative for fiscal balance, revenue, and
capital outlays.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{figure}[h!]
  ...
  \footnotesize
  \textit{Notes}: Doubly-robust C\&S dynamic ATT estimates by event time
  $\ell \in \{-10, -5, 0, 5\}$ corresponding to CoG census years 2002, 2007, 2017/2022,
  and 2022 relative to the treatment cohort year. Left panel: fiscal balance per capita.
  Right panel: capital expenditures per capita. Vertical lines are 95\% bootstrap
  confidence intervals (clustered by county). Dashed vertical line at $\ell = -2.5$
  marks the pre/post divide. Pre-treatment joint test: $p > 0.05$ for both outcomes.
  \end{minipage}
\end{figure}
